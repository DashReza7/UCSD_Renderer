...
vec3 Renderer::get_pixel_color_pathtrace(const Ray &r, uint32_t depth, vec3 incoming_throughput)
{
    ...
    Shape *hit_shape = static_cast<Shape *>(rec.hit_obj);
    vec3 color{};
    ...
    if (scene->next_event_estimation)
        for (AreaLight al: scene->areaLights)
        {
            ...
            for (int i = 0; i < scene->light_samples; ++i)
            {
                ...
                is_hit = scene->use_bvh ? scene->bvh->hit(shadow_ray, T_MIN, T_MAX, foo_rec) : hit_brute_force(shadow_ray, T_MIN, T_MAX, foo_rec);
                if (is_hit && foo_rec.t < light_dist) // if the area_light is in shadow, ignore this area_light source
                    continue;
    ...
    // indirect light
    Ray bounced_ray{rec.hit_pos + rec.normal * EPS,
                    sample_dirn(uniform_dis(gen) <= hit_shape->mat.get_reflectivity(), hit_shape, r.direction * -1.0f, rec.normal)};
...
float Renderer::get_pdf(ImportanceSamplingType importance_sampling_type,
                        const vec3 &normal,
                        const vec3 &w_i,
                        const vec3 &w_o,
                        const Shape *hit_shape) const
{
    float pdf = 0.0f;
    if (importance_sampling_type == ImportanceSamplingType::UNIFORM_HEMISPHERE)
    {
        pdf = 1.0f / (2.0f * std::numbers::pi);
    }
    else if (importance_sampling_type == ImportanceSamplingType::COSINE)
    {
        // TODO: division by this pdf might raise DivisionByZero Error!
        pdf = dot(normal, w_i) / std::numbers::pi;
    }
    else if (importance_sampling_type == ImportanceSamplingType::BRDF)
    {
        ...
        else if (hit_shape->mat.brdf_type == BRDF_TYPE::GGX)
        {
            ...
            float tan_sqrd_h = dot(normal, half_vector) != 0 ? 1.0f / (std::pow(dot(normal, half_vector), 2)) - 1.0f : 1.0f;
...
vec3 Renderer::sample_dirn(bool sample_specular,
